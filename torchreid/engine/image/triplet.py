...
from torchreid.losses import CrossEntropyLoss, TripletLoss, Isolate_loss
...
class PoseTripleEngine(engine.Engine):
    def __init__(self, datamanager, model, optimizer, margin=0.3,
                     weight_t=1, weight_x=1, scheduler=None, use_cpu=False,
                 label_smooth=True):
        super(PoseTripleEngine, self).__init__(datamanager, model, optimizer, scheduler, use_cpu)
        self.weight_t = weight_t
        self.weight_x = weight_x
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            outputs, features, attmaps = self.model(imgs, pose_heatmaps)
